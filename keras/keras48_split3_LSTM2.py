...
x, y = split_xy(a,size)
x = x.reshape(-1,2,2)
x_predict = split2(x_predict,size).reshape(-1,2,2)

print(x,y)
print(x.shape,x_predict.shape)  # (96, 2, 2) (7, 2, 2)

x_train, x_test, y_train, y_test = train_test_split(x,y,test_size=0.1,random_state=333)

model = Sequential()
model.add(LSTM(512,input_shape=(2,2),return_sequences=False,activation='relu'))
